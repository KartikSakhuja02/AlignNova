...
export default function AdminDashboard() {
  ...
  const [testEmailResult, setTestEmailResult] = useState(null); // null | { ok, provider, error, config }
      ...
      <main className="flex-1 md:ml-64 min-h-screen pb-20">
        ...
        <div className="max-w-[1400px] mx-auto p-p-lg space-y-10">
          ...
          <section className="bg-white rounded-2xl border border-outline-variant shadow-sm overflow-hidden">
            <div className="p-p-lg border-b border-outline-variant flex items-center gap-3">
              <div className="w-10 h-10 bg-primary-container/20 text-primary rounded-lg flex items-center justify-center">
                <span className="material-symbols-outlined">mail</span>
              </div>
              <div>
                <h2 className="text-headline-md font-bold text-on-surface">Email Diagnostics</h2>
                <p className="text-caption text-on-surface-variant">Verify email delivery is working correctly</p>
              </div>
            </div>
            <div className="p-p-lg grid grid-cols-1 md:grid-cols-2 gap-8">
              {/* Config status */}
              <div>
                <p className="text-label-md font-bold text-on-surface mb-3">Current Configuration</p>
                {emailConfig ? (
                  <div className="space-y-2">
                    {/* Provider badge */}
                    <div className="flex items-center gap-2">
                      <span className="text-caption text-outline w-32">Provider</span>
                      <span className={`px-2.5 py-1 rounded-lg text-caption font-bold ${
                        emailConfig.provider === 'none'
                          ? 'bg-error-container/20 text-error'
                          : 'bg-secondary-container/20 text-secondary'
                      }`}>
                        {emailConfig.provider === 'gmail_smtp' ? '✅ Gmail SMTP'
                          : emailConfig.provider === 'resend' ? '✅ Resend API'
                          : '❌ Not configured'}
                      </span>
                    </div>
                    {emailConfig.provider === 'gmail_smtp' && (
                      <>
                        <div className="flex items-center gap-2">
                          <span className="text-caption text-outline w-32">SMTP User</span>
                          <span className="text-caption text-on-surface font-mono">{emailConfig.smtp_user}</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <span className="text-caption text-outline w-32">App Password</span>
                          <span className={`text-caption font-bold ${
                            emailConfig.smtp_pass_length === 16 ? 'text-secondary' : 'text-error'
                          }`}>
                            {emailConfig.smtp_pass_set
                              ? `${emailConfig.smtp_pass_length} chars ${ emailConfig.smtp_pass_length === 16 ? '✅' : '⚠️ should be 16'}`
                              : '❌ Not set'}
                          </span>
                        </div>
                      </>
                    ...
                    {emailConfig.provider === 'resend' && (
                      <div className="flex items-center gap-2">
                        <span className="text-caption text-outline w-32">API Key</span>
                        <span className="text-caption text-on-surface font-mono">{emailConfig.resend_key_prefix}</span>
                      </div>
                    )}
                    <div className="flex items-center gap-2">
                      <span className="text-caption text-outline w-32">From Email</span>
                      <span className="text-caption text-on-surface font-mono">{emailConfig.from_email}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-caption text-outline w-32">App Base URL</span>
                      <span className="text-caption text-on-surface font-mono truncate max-w-[200px]">{emailConfig.app_base_url}</span>
                    </div>
                    {emailConfig.provider === 'none' && (
                      <div className="mt-3 p-3 bg-error-container/10 border border-error/20 rounded-xl">
                        <p className="text-caption text-error font-semibold">Set SMTP_USER + SMTP_PASS (Gmail) or RESEND_API_KEY in Render Environment Variables.</p>
                      </div>
                    )}
                  </div>
                ) : (
                  <div className="flex items-center gap-2 text-outline text-caption">
                    <svg className="animate-spin h-4 w-4" fill="none" viewBox="0 0 24 24">
                      <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                      <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
                    </svg>
              ...
              {/* Send test */}
              <div>
                <p className="text-label-md font-bold text-on-surface mb-3">Send Test Email</p>
                <form onSubmit={handleTestEmail} className="space-y-3">
                  <div className="flex gap-2">
                    <input
                      ...
                      required
                      className="flex-1 border border-outline-variant rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none px-4 py-2.5 text-body-md bg-surface-container-lowest transition-all"
                    />
                    <button
                      type="submit"
                      disabled={testEmailLoading || !testEmailAddr}
                      className="px-5 py-2.5 bg-primary text-on-primary text-label-md font-semibold rounded-xl hover:scale-[1.02] active:scale-95 transition-all disabled:opacity-50 flex items-center gap-2"
                    >
                      {testEmailLoading ? (
                        <svg className="animate-spin h-4 w-4" fill="none" viewBox="0 0 24 24">
                          <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                    ...
                    <div className={`p-4 rounded-xl border text-sm ${
                      ...
                      {testEmailResult.ok ? (
                        <p className="text-secondary font-semibold flex items-center gap-2">
                          ...
                          Email sent successfully via {testEmailResult.provider}! Check your inbox.
                        </p>
                      ) : (
                        <div>
                          <p className="text-error font-bold flex items-center gap-2 mb-2">
                            <span className="material-symbols-outlined" style={{ fontVariationSettings: "'FILL' 1", fontSize: '18px' }}>error</span>
                            Failed to send email
                          </p>
                          <p className="text-error/80 text-xs leading-relaxed font-mono whitespace-pre-wrap break-words">
